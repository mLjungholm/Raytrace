...
%                 receptor_view_center(receptor_ind,:) = sph2pixel(absorption_results.view_dir(receptor_ind,:)./180*pi,[imageSize imageSize],optics_calibration_file);
                [yi,xi] = sph2pixel(absorption_results.view_dir(receptor_ind,:)./180*pi,[imageSize imageSize],this.diamMod*this.image_size);
            ...
            for source_ind = 1:this.source_nums
                ...
                [yi,xi] = sph2pixel([el,az],[imageSize imageSize],round(imageSize*this.diamMod));
...
%             We also reduce to single pecision for reduced memmory usage
            ...
            for receptor_ind = 1:this.receptor_nums
                if this.down_supersampling
                    interpolated_image = griddata(this.coord_transfer_mat(:,2),this.coord_transfer_mat(:,1),absorption_results.absorption_mat(:,receptor_ind).*this.weights,y_grid,x_grid);
                else
                    interpolated_image = griddata(this.coord_transfer_mat(:,2),this.coord_transfer_mat(:,1),absorption_results.absorption_mat(:,receptor_ind),y_grid,x_grid);
                end
            ...
            theshold_val = 0.1;
            this.edge_filter = griddata(this.coord_transfer_mat(:,2),this.coord_transfer_mat(:,1),absorption_results.total_abs,y_grid,x_grid);
